...
int outaudio_free() {
    ...
    fprintf(stderr, "Killing outaudio\n");
    kill(pid_outaudio, SIGKILL);
    waitpid(pid_outaudio, NULL, 0);
    fprintf(stderr, "Killed outaudio\n-----\n");
    pid_outaudio = -1;
    return 0;
}
...
int outaudio_init() {
    ...
    char outcommand[COMMAND_LENGTH] = "sox --buffer 1024 -S";
    // "sox --buffer 1024 -S -m -r 44100 -c 1 -n";
    int filecount = 0;
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i] != NULL) {
            filecount++;
    ...
    for (int i = 0; i < MAX_CLIENTS || last_index >= COMMAND_LENGTH; i++) {
        if (clients[i] != NULL) {
            last_index += snprintf(outcommand + last_index, COMMAND_LENGTH - last_index, " -t s16 -c 1 -r 44100 %s", clients[i]->filename);
        }
    }
    last_index += snprintf(outcommand + last_index, COMMAND_LENGTH - last_index, " -d");
    if (last_index >= COMMAND_LENGTH) {
        fprintf(stderr, "Command too long\n");
        return -1;
    }
    // CHANGE HERE
    pid_outaudio = fork();
    if (pid_outaudio == 0) {  // Child
        fprintf(stderr, "Executing: %s\n", outcommand);
        execl("/bin/bash", "bash", "-c", outcommand, NULL);
    }
    if (pid_outaudio == -1) {
        perror("fork");
        return -1;
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        ClientSettings *client = clients[i];
        if (client != NULL) {
            fprintf(stderr, "Opening %s\n", client->filename);
            client->fd = open(client->filename, O_WRONLY);
            fprintf(stderr, "Opened %s -> result: %d\n", client->filename, client->fd);
        }
    }
    fprintf(stderr, "-----\n");
    return 0;
